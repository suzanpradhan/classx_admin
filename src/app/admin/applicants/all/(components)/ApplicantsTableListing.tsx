...
const ApplicantsTableListing = () => {
    ...
    const applicantsData = useAppSelector(
        (state: RootState) =>
            state.baseApi.queries[`getAllApplicants`]?.data as PaginatedResponseType<ApplicantsType>
    );
    return (
        <>
            <AlertDialog
                ...
                onClickYes={async () => {
                    if (onDelete) {
                        await Promise.resolve(
                            dispatch(applicantsApi.endpoints.deleteApplicants.initiate(onDelete as string))
                        );
                    }
                    toggleDeleteModel(false);
