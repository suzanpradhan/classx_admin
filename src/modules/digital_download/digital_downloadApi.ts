  ...
  endpoints: (builder) => ({
    ...
    addigital: builder.mutation<any, Digital_DownloadSchemaType>({
      query: (payload) => {
        ...
        if (payload.max_downloads)
          formData.append('max_downloads', payload.max_downloads.toString());
        if (payload.release) {
    ...
    // Get All
    getAlldigital: builder.query<
      PaginatedResponseType<Digital_DownloadType>,
      string
    >({
      query: (pageNumber) =>
        `${apiPaths.digital_downloadUrl}?page=${pageNumber}`,
    ...
    deleteDigital: builder.mutation<any, string>({
      ...
      invalidatesTags: (result, error, id) => [
        { type: 'Digital_Download', id },
      ],
    }),

    // Update
    updatedigital: builder.mutation<
      Digital_DownloadType,
      Digital_DownloadSchemaType
    >({
      query: ({ id, ...payload }) => {
