...
const genresApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAllGenres: builder.query<
      PaginatedResponseType<GenresType>,
      { pageNumber: string; searchString?: string }
    >({
      query: ({ pageNumber, searchString }) =>
        `${apiPaths.genresUrl}?page=${pageNumber}${searchString ? `&search=${searchString}` : ''}`,
    ...
    getEachGenres: builder.query<GenresType, string>({
      query: (property_typeId) => `${apiPaths.genresUrl}${property_typeId}/`,
    ...
    updateGenres: builder.mutation<GenresType, GenresSchemaType>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'Genres', id: id! }],
    }),
  }),
