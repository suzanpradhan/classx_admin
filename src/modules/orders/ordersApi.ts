...
const ordersApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // Get All
    getAllOrders: builder.query<PaginatedResponseType<OrdersType>, number>({
      query: (pageNumber) => `${apiPaths.ordersUrl}?page=${pageNumber}`,
    ...
    getEachOrders: builder.query<OrdersType, string>({
      query: (id) => `${apiPaths.ordersUrl}${id}/`,
    ...
    // Update
    updateOrders: builder.mutation<OrdersType, OrdersSchemaType>({
      query: ({ id, ...payload }) => {
        const formData = new FormData();
        formData.append('total_amount', String(payload.total_amount));
        formData.append('billing_city', payload.billing_city);
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'Orders', id: id! }],
    }),
  }),
