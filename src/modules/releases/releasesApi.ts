...
const releaseApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAllReleases: builder.query<
      PaginatedResponseType<ReleasesType>,
      { pageNumber: string; searchString?: string; artistSlug?: string }
    >({
      query: ({ pageNumber, searchString, artistSlug }) =>
        `${apiPaths.releasesUrl}?page=${pageNumber}${searchString ? `&search=${searchString}` : ''}${artistSlug ? `&artist__slug=${artistSlug}` : ''}`,
      ...
      serializeQueryArgs: ({ endpointName, queryArgs }) => {
        return endpointName + (queryArgs.artistSlug ?? '');
      },
      async onQueryStarted(payload, { queryFulfilled }) {
        try {
          await queryFulfilled;
    ...
    getEachReleases: builder.query<ReleasesType, string>({
      query: (releasesId) => `${apiPaths.releasesUrl}${releasesId}/`,
    ...
    deleteReleases: builder.mutation<any, string>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'Releases', id: id! }],
    }),
